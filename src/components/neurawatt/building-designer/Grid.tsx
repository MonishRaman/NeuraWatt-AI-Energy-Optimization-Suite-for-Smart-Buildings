...
export const Grid = ({ selectedRoomType, rows = 10, cols = 15 }: GridProps) => {
        ...
        {grid.map((row, rowIndex) => (
          <div key={rowIndex} className="flex">
            {row.map((cell, colIndex) => (
              <div
                key={cell.id}
                className={`w-12 h-12 border border-gray-200 cursor-pointer transition-colors flex items-center justify-center text-xs`}
